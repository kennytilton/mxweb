...
class Cell {
	...
	calcNLink() {
		...
		try {
            //clg(`stacking ${this.name} rule with md ${this.md} `+this.rule.toString());
			callStack.push(this);
	...
	observerResolve () {
		...
		// if (!this.observer) console.log('nope obs');

		/*if ((!this.observer) && this.md) { // The Model class
			console.log('obsresing '+typeof this);
			this.observer = this.md.slotObserverResolve(this.name);
		}*/
        //console.log('obs is='+this.observer+' '+ typeof this.observer);
		if (this.observer===null) {
			if (this.md) {
				this.observer = this.md.slotObserverResolve(this.name);
				hunh = this.observer;
			} else {
				this.observer = kObserverUnresolved;
			}
        }
        //console.log('obs is='+hunh+','+typeof hunh);
        if (this.observer===kObserverUnresolved) return null;
	...
	observe( vPrior, tag) {
		...
		if (obs) {
			//console.log('bingo obs='+this.name+' '+typeof obs);
