...
function todoFRP() {
    console.log('todoFRP!!!!!!!');
    // bulkTodo("hi mom", 2000);

    // todoSession.init();
    // return Tag.mxToHTML( [h1("Todos!")]);
...
console.log('window todofrp assigned!');

function bulkTodo (tag, ct) {
    var ts = [];
    for ( var n=0; n<ct; ++n) {
        ts.concat( new Todo( {title: tag+"-"+n}));
    }
    Todos.itemsRaw = Todos.itemsRaw.concat(ts);
}
...
function todoAddNew (dom, e) {
    ...
    if (title !== '') {
        ...
        console.log( 'elapsed= '+ (Date.now() - go));
    }
    e.target.value = null;
}
